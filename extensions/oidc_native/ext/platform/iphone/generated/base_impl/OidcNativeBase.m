...
-(void) getProperty:(NSString*)propertyName methodResult:(id<IMethodResult>)methodResult {
    NSObject* value = [mProperties objectForKey:[OidcNativeBase applyAliasesToPropertyName:propertyName]];
...
-(void) setProperty:(NSString*)propertyName propertyValue:(NSString*)propertyValue methodResult:(id<IMethodResult>)methodResult {
    ...
    if ([value isKindOfClass:[NSNumber class]]) {
        ...
        if ([CMethodResult isBoolInsideNumber:numValue]) {
            ...
            // error !
            NSLog(@"OidcNative.setProperty(\"%@\", value) UNSUPPORTED VALUE TYPE ! MUST BE STRING !!!", propertyName);
        }
    }
   ...
   [mProperties setObject:strValue forKey:[OidcNativeBase applyAliasesToPropertyName:propertyName]];
}
